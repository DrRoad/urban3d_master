    ...
    if model_name == "UNet":
        ...
        if args.num_channels > 3:
            ...
            with torch.no_grad():
                ...
                model.encoder.conv1.weight[:, 3] = model.encoder.conv1.weight[:, 0]
        return model
    elif model_name == "Linkenet":
        print("using FPN")
        model = smp.Linknet(encoder_name='resnet50', classes=num_classes)
        if args.num_channels > 3:
            weight = model.encoder.conv1.weight.clone()
            model.encoder.conv1 = torch.nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3, bias=False)
            with torch.no_grad():
...
############## the main function #################
def main():
    outPath = f"{args.results}_{args.model_name}_YpUnet"
    ...
    if args.pre_train:
        net = torch.load(args.ckp)["model_state"]

    ########## When the number of GPU is more than one, the function (DataParallel) will be appiled. #############
    ...
    #################when the metric is better than before, the model will be saved###########
        if val_hist["miou"] > best_miou:
            ...
            torch.save(state, os.path.join(outPath, checkpoint))
...
if __name__ =="__main__":
    ...
    parse.add_argument("--model_name", type=str, default="YpUNet", help='YpUNet, Linknet, FPN, UNet , PSPNet')
